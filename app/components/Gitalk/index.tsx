...
import { GT_ACCESS_TOKEN, GT_COMMENT, GT_VERSION } from './const';
...
class GitalkComponent extends Component {
  ...
  comments() {
    const { user, comments, isLoadOver, isLoadMore, pagerDirection } =
      this.state;
      ...
      <div className="gt-meta" key="meta">
        ...
        <div className="gt-user">
          {user ? (
            <div
              className={
                isPopupVisible ? 'gt-user-inner is--poping' : 'gt-user-inner'
              }
  ...
  render() {
    const { isIniting, isNoInit, isOccurError, errorMsg, isInputFocused } =
      this.state;
    return (
      <div
      ...
      >
        ...
        {!isIniting &&
          (isNoInit ? [this.noInit()] : [this.header(), this.comments()])}
      </div>
    );
  }
}
